...
{
	// private members
	var data, url,urlPageGet,urlItemGet,urlItemSet, template 
		form  = $('<form/>'),
		table = $('<table/>' , {
			class : 'table t-hover'
		}),
	...
	function onCreate()
	{
		document.location.href = 'create.html?url='+url+'.json';		
	}

	// open change page with specific data
	function onChange(id)
	{
		document.location.href = 'change.html?id='+id+"&url="+url+'.json';
	}

	function onView(id)
	{
		document.location.href = 'view.html?id='+id+"&url="+url+'.json';
	}
	...
	function request(page=1 , callback=null)
	{
		$.ajax( {
			// url			: url+'.json',
			url			: 'backend/books.get.php',
			method		: 'GET',
			dataType	: 'json',
			data		: {
				page	:	1
	...
	// return table for change data in callback function
